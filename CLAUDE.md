...
- Stage 4 (ROOT のみ) は CLI: `aget-shaper -i in_500V_readout.root` (既定 `-t 223` ns, `-r 120` fC, `-g 1000`, `-p 450`, `-s 6`)。出力は `_raw.root` で、中身は実機と同じ `raw` ツリー。`external/tpcdaq-macros/` の解析マクロ (analyzeUVW.C → makeTracks.C) がそのまま走る
- `geometry/pads.csv` と `channel_map.csv` は実機ジオメトリ由来なので git に入れない。channel_map.csv は `scripts/make_channel_map.py geometry/pads.csv channel_map.csv` で pads.csv から作る。pads.csv が無い環境でもビルドと Stage 1〜2 のテストは通る (Stage 3〜4 のテストは登録されず警告が出る)
