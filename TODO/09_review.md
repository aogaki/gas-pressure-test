...
前提: TODO/01〜08。状態: レビューのみ (コードは未変更、全 112 テスト合格のまま)。修正・リファクタリングは次回、この文書から発注書 (TODO/10) を書いて行う。
...
**R21 ✔ 公開リポジトリに実機ジオメトリ由来の数値がある (要判断)**
...
- 判断は発注者に委ねる。外すなら、先頭パッドの座標と割当の代わりに「全パッドの辺長が 1.000 mm」「全 (aget, ch_graw) が 256 通り」など座標を含まない不変条件に置き換える
