...
- 受け入れテストの `build/tests/acceptance/` は hits true の 3 ラン (各 1000 イベント) で約 600 MB を使っていた。生成閾値 10 m 後は 1 ラン 12 MB

2026-09-07、生成閾値を 10 m に変更後 (defaultCutValue、下記タスク 2)。条件は上表と同じ
(Apple Silicon、Release、Ar 200 mbar、5.5 MeV、10000 イベント、`/random/setSeeds 1 1`)。

| `/tpc/hits` | 実時間 [s] | イベント/秒 | 出力 [MB] | 1 イベントあたり |
|---|---|---|---|---|
| false | 0.63 | 15900 | 0.65 | 65 B |
| true | 4.04 | 2480 | 118 | 11.8 kB |

- hits の行はすべて pdg 1000020040 (α) で、pdg 11 (e-) の行は無くなった (δ 線を独立飛跡として作らないため)。1 イベントあたり約 234 行 (α 本体の 1 mm 刻みのみ)
- イベント/秒は生成閾値変更前 (上表) と比べて hits false で約 33 倍、hits true で約 15 倍になった (δ 線の生成と追跡が無くなったため)
...
- 生成閾値を 10 m にした後の実測は hits false で 15900 イベント/秒 (Ar 200 mbar 5.5 MeV)。1M イベントで約 63 秒、65 MB
- 決定 (2026-09-07): エネルギーは 0.3 MeV と 0.5 から 10 MeV の 0.5 刻み (21 点)、1 ラン 1M イベント。3 ガス × 5 圧力なら 315 ランで、14 プロセス並列なら約 25 分。運用は TODO/04
- hits true で 1M イベントは 1 ファイル数十 GB になるので、hits はドリフトに使う少数のランに限る
- 代案 (採用せず): GPS の一様スペクトル (`/gps/ene/type Lin`, gradient 0) で 0.3 から 10 MeV を 1 ランで走らせ、e0 でビン分けする。速くなった今は不要だが、AT-11 で動作確認済みなのでいつでも使える
